...
var winSelect = [];
var g_iWndIndex_forw = 0; //YF保存切换窗口数目前选中的窗口
var iType_forw = 0;//YF:保存切换窗口数目前窗口的数目
...
$(function () {
	...
	WebVideoCtrl.I_InitPlugin('100%', '100%',{
        ...
        cbSelWnd: function (xmlDoc) {
			...
			var szInfo = "yf当前选择"+g_iWndIndex+"号窗口";
			showCBInfo(szInfo);
		}
	});
...
function showOPInfo(szInfo) {
    $("#messageDiv").prepend(szInfo + '</br>');
}

// 显示回调信息
function showCBInfo(szInfo) {
	szInfo = dateFormat(new Date(), "hh:mm:ss") + ":" + szInfo;
    $("#messageDiv").prepend(szInfo + '</br>');
}
...
function clickOpenFileDlg(id, iType) {
    ...
    arrXml.push("<CapturePath>" + $("#recordPath").val() + "</CapturePath>");
    arrXml.push("</LocalConfigInfo>");
...
function changeWndNum(iType){
	...
	g_iWndIndex_forw = g_iWndIndex;

	//YF测试：窗口数为1时显示选中的窗口画面
	//多窗口变一窗口
	if((iType==1)&&(g_iWndIndex!=0))  // 窗口数为1，且当前选择的不是0号窗口，需要显示当前选择的窗口
	{
		// isChangeWnd = 1;
		// g_iWndIndex_forw = g_iWndIndex;
		//clickStopRealPlay();
       ...
       // setTimeout(clickStartRealPlay(),5000);
		g_iWndIndex = 0; //直接0号窗显示，此处自动获取有时是0号，有时是之前的窗口（窗口数切换函数有延迟）
		clickStartRealPlay();
		iType_forw = iType;
	}
		//一窗口变多窗口
...
function clickLogin(){
    ...
    var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
        success: function (xmlDoc) {
            ...
            $("#ip").prepend("<option value='" + szIP + "'>" + getNameBySxtIp(szIP) + "</option>");
            setTimeout(function () {
                $("#ip").val(szIP);
                getChannelInfo();
            }, 10);
...
function settingDialog() {
    var width=900;
    var height=600;
    var y=(window.screen.availHeight-600);
    var x=(window.screen.availWidth-900);
    var mywindow = window.open("index/toSettings", "_blank","height="+height+",width="+width);
...
function clickStartRealPlay(){
	...
	szInfo = "ceshi当前选择"+g_iWndIndex+"号窗口";
	showCBInfo(szInfo);
	szInfo = "";
	//设置图片路径   录像
    var path = "C:/Users/rushi/Desktop/demo";
   // var recordPath  ="C:/Users/rushi/Desktop/demo/record"
    var arrXml = [],
		szInfo = "";
    arrXml.push("<CapturePath>" + path + "</CapturePath>");
   // arrXml.push("<RecordPath>" + recordPath + "</RecordPath>");
    var iRet = WebVideoCtrl.I_SetLocalCfg(arrXml.join(""));
...
function clickStartRecord() {
	...
	$("#startRecord").attr("disabled","disabled");
...
function getNameBySxtIp(sxtIp) {
    console.log(ytClientData);
    for(var i=0;i<ytClientData.length;i++){
    	console.log(sxtIp);
    	if(ytClientData[i].sxtIp == sxtIp){
    		return ytClientData[i].relationName
		}
	}
	return "此设备暂无名称";
}